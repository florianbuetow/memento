...
> `$MEMENTO_ENV/skills/audio/transcription/speech_to_text/`. The canonical format
...
- Skill name is `snake_case`. Either describes the target (`to_mp3`,
  `to_wav`) or names the method (`from_url`, `speech_to_text`).

For our example: `audio/transcription/speech_to_text/`. We added a new
...
`$MEMENTO_ENV/skills/audio/transcription/speech_to_text/SKILL.md`.
...
What we validate for `speech_to_text`:
...
For `speech_to_text` the helper computes the output paths using only the
...
Outputs section. For `speech_to_text`: both output files exist, are
