...
def test_real_library() -> None:
    source = "video/download/from_url"
    ...
    check(
        "real: transcript chain goes through speech_to_text",
        "audio/transcription/speech_to_text" in result.stdout,
        result.stdout,
    )
    ...
    result = run_finder(REAL_NODES, REAL_EDGES, source, "images/viewing/slideshow")
    if image_producers:
        check(
