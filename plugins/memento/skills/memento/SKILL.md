...
2. Read `$MEMENTO_ENV/graph/skills_nodes.txt` and map the goal to two endpoints:
   ...
   - Types are the TYPE values already present in that nodes file: its `inputs=` and `outputs=` fields list every type the library actually uses. Example: "download video [url] and extract the transcript" → `--from skill:video/download/from_url --to type:TEXT_FILE_TXT`.
