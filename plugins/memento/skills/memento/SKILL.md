...
2. Read `$MEMENTO_ENV/graph/skills_nodes.txt` and map the goal to two endpoints:
   ...
   - Types are the TYPE values from `$MEMENTO_ENV/resources/skill_guide.md`. Example: "download video [url] and extract the transcript" → `--from skill:video/download/from_url --to type:TEXT_FILE_TXT`.
