...
2. Read `$MEMENTO_ENV/graph/skills_nodes.txt` and map the goal to two endpoints:
   ...
   - Types are the TYPE values already present in that nodes file: its `inputs=` and `outputs=` fields list every type the library actually uses. Example: "convert the recording [file] to mp3 and extract the transcript" → `--from skill:audio/conversion/to_mp3 --to type:TEXT_FILE_TXT`.
